...
<aui:form name="frm" method="POST" action="<%=danhSachURL%>">
	<table>
		<tr>
			...
			<td><aui:button type="submit"  icon="icon-search" style="width: 101px;margin-bottom: 12px;" value="Tìm kiếm"></aui:button></td>
		</tr>
	</table>
</aui:form>
<liferay-portlet:actionURL var="deleteURL" name="delete"></liferay-portlet:actionURL>
<aui:form  method="post"  action="<%= deleteURL %>" name="fm">
<liferay-ui:search-container iteratorURL="<%=interatorURL%>"
	...
	<liferay-ui:search-container-row className="danhmuc.model.Sach"
		...
		<liferay-ui:search-container-column-text name="Chức năng">
			...
			<aui:button value="Cập Nhât" icon="icon-edit" href="<%=updateURL %>"></aui:button>
			<liferay-portlet:actionURL var="chitietURL" name="chitiet">
				<liferay-portlet:param name="id" value="<%=String.valueOf(item.getId())%>"/>
			</liferay-portlet:actionURL>
			<aui:button value="Thông tin" icon="icon-zoom-in" href="<%=chitietURL %>"></aui:button>
		</liferay-ui:search-container-column-text>
	</liferay-ui:search-container-row>
	<liferay-ui:search-iterator paginate="true"/>
	</liferay-ui:search-container>
	 <aui:button value="delete" type="button"  onClick='<%= renderResponse.getNamespace() + "showSelectedFoos();"  %>' />
...
<aui:script>
	Liferay.provide(
		...
		function() {
			...
			alert(deleteStudentIds);
			if (deleteStudentIds && confirm('<%= UnicodeLanguageUtil.get(pageContext, "Bạn thực sự muốn xóa nội dung này") %>')) {
